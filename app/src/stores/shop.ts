  ...
  actions: {
    async fetchTypes(this: any) {
      if (Object.keys(this.types).length === 0) {
        try {
          const response: AxiosResponse<{ types: Type }> = await axios.get(
      ...
      if (!this.products[`${type}_${category}`]) {
        try {
          const response: AxiosResponse<{ products: Products }> = await axios.get(
            `http://localhost:3000/shop/${type}/${category}`
          )
      ...
      if (!this.product || !this.product[`${type}_${category}_${name}`]) {
        try {
          const response: AxiosResponse<Products> = await axios.get(
            `http://localhost:3000/shop/${type}/${category}/${name}`
          )
